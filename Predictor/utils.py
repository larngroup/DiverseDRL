...
def data_division(config,smiles_int,labels,cross_validation,model_type,descriptor):
    ...
    if model_type != 'dnn' or descriptor == 'ECFP':
        msk = np.random.rand(len(smiles_int)) < 0.9
        labels_df = pd.DataFrame({'lbs': labels[:]})
        
        labels_df_train_val = labels_df[msk]
        labels_df_test = labels_df[~msk]
    
        train_val_set = smiles_int[msk]
        test_set = smiles_int[~msk]
        
        data.append(train_val_set)
        data.append(labels_df_train_val.to_numpy())
        data.append(test_set)
        data.append(labels_df_test.to_numpy())
    else:
